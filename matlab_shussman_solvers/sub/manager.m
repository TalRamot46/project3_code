...
%Solve the constants
B=(16*mat.sigma*mat.g)/(mat.beta)/3;B=B*((mat.r*mat.f)^(-(4+mat.alpha)/mat.beta));
...
Ptilda=x(1,3);
utilda=x(end,5);
P0=Ptilda*(B^Pw(1))*((mat.r*mat.f)^(Pw(2)/mat.beta))*(10^(-9*(-Pw(2)*tau)))/1e12*1160500^Pw(2)*(1e-9)^Pw(3); %g/cm???2
m0=xsi*(B^mw(1))*((mat.r*mat.f)^(mw(2)/mat.beta))*(10^(-9*(-mw(2)*tau)))*1160500^mw(2)*(1e-9)^mw(3); %g/cm???2
e0=z*(B^ew(1))*((mat.r*mat.f)^(ew(2)/mat.beta))*(10^(-9*(-ew(2)*tau)))*1160500^ew(2)*(1e-9)^ew(3)/1e9/100; %J/cmA2
V0=(B^Vw(1))*((mat.r*mat.f)^(Vw(2)/mat.beta))*(10^(-9*(-Vw(2)*tau)))*1160500^Vw(2)*(1e-9)^Vw(3); %g/cm???2
u0=utilda*(B^uw(1))*((mat.r*mat.f)^(uw(2)/mat.beta))*(10^(-9*(-uw(2)*tau)))/1e5*1160500^uw(2)*(1e-9)^uw(3); %g/cm???2
end
