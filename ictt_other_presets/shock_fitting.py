# ictt_other_presets/shock_fitting.py
"""
verification and similarity profile fitting script for the Piston Shock regime.

Outputs generated inside results/ictt_other_presets/ (split by graph types):
...
def get_cached_shock_solver(case, case_label):
    ...
    p0 = _ns_amplitude_rescale(float(case.P0_Barye) if case.P0_Barye is not None else 1.0, tau)
    # Check if case has P0_Barye; if Marshall, P0_Barye is None, we need to handle it or extract it.
    # Wait, for Fig 9 and Fig 10, does AblationSolver or PistonShock solver use P0_Barye or get it from Case?
    # Let's see: for Marshak case, the pressure P_piston is determined by the radiation heat wave!
    # Wait! In full_fitting.py, shock_solver is instantiated inside AblationSolver, which coordinates
    # the piston pressure between subsonic heat wave and piston shock.
    # If we run shock_fitting.py as a standalone script, it solves PistonShock. But PistonShock needs p0 (drive pressure amplitude).
    # Wait! In shock_fitting.py:
    # case.P0_Barye is used. But for Marsahk cases (Fig 9, Fig 10), case.P0_Barye is None!
    # Ah! Let's check how AblationSolver sets P0 for shock solver.
    # In project3_code/menahem_new/ablation_solver_og.py, it does:
    # self.shock_solver = PistonShock(rho0=self.rho0, omega=self.omega, p0=self.heat_solver.P0_dimensional, tau=self.tau, gamma=self.gamma)
    # Ah! The pressure amplitude for the shock is `self.heat_solver.P0_dimensional`!
    # Let's verify how it is solved. The heat solver solves similarity ODEs and finds `P0` (dimensionless pressure at origin).
    # Then `P0_dimensional = P0 * (A^a3) * (B^b3)`.
    # So if case.P0_Barye is None, we can solve the subsonic heat wave solver first to find the piston pressure,
    # and then pass that as p0 to the shock solver!
    # Let's check if we can do that in `get_cached_shock_solver` or if case has a fallback.
    # Wait! Let's check how `project3_code/rad_hydro_sim/verification/menahem_comparison.py` sets shock kwargs.
    # Let's do a search or view `_shock_kwargs_from_case`.
    
    # Let's inspect `_shock_kwargs_from_case` or see if there is any code handling it.
    # Let's write a robust way in `get_cached_shock_solver` to handle `P0_Barye is None`:
    # If P0_Barye is None, we run SubsonicHeatWave shooting first to find P0_dimensional, and use it as p0!
    # That is extremely clever and robust!
...
def perform_shock_fitting(solver):
    ...
    # Construct piecewise composite profiles
    if FITTING_OPTION == "FIT_RHO_ALL_AROUND":
        low_mask = y_valid < 0.0
        high_mask = np.ones_like(y_valid, dtype=bool)
    else:
...
def run_simulation_and_references(preset_name: str, case_label: str):
    ...
    cache_dir = Path("results/ictt/cache")
    cache_dir.mkdir(parents=True, exist_ok=True)
    cache_path = cache_dir / f"{case_label}_cache_shock.pkl"
...
def get_plot_paths(case_label: str) -> dict[str, str]:
    ...
    ss_shock_dir = case_dir / "self_similar_shock"
    dv_dir = case_dir / "dimensional_verification"
    ss_shock_dir.mkdir(parents=True, exist_ok=True)
    dv_dir.mkdir(parents=True, exist_ok=True)

    return {
        "self_similar": str(ss_shock_dir / f"{case_label}_self_similar.png"),
        "velocity_fits": str(ss_shock_dir / f"{case_label}_standalone_velocity.png"),
        "temperature_fits": str(ss_shock_dir / f"{case_label}_standalone_temperature.png"),
        "relative_errors": str(ss_shock_dir / f"{case_label}_relative_error.png"),
        "dimensional_comparison": str(dv_dir / f"{case_label}_dimensional_shock.png")
    }
